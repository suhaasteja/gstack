...
description: |
  ...
  "show now playing in Claude". Claude Code only — uses the statusLine settings.json
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"statusline-setup","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"statusline-setup","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"statusline-setup","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /statusline-setup — Claude Code Status Bar
...
- ♫ Spotify now playing (macOS only — silently omitted on Linux/Windows)
...
- **macOS:** Spotify integration requires granting Terminal Automation permission for
  Spotify in System Settings → Privacy & Security → Automation. Without it, the ♫
